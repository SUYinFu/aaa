...
plateName = r"#京沪津渝冀晋蒙辽吉黑苏浙皖闽辽鲁豫鄂湘辽辽琼辽贵云藏陕甘青宁新学警港澳挂使领民航危0123456789ABCDEFGHJKLMNPQRSTUVWXYZ险品"
...
def cv2ImgAddText(img, text, left, top, textColor=(0, 255, 0), textSize=20):  # 将识别结果画在图上
    ...
    fontText = ImageFont.truetype(
        "assets/fonts/platech.ttf", textSize, encoding="utf-8")
...
def draw_result(orgimg, dict_list):
    ...
    for result in dict_list:
        if result['plate_color'] == '黄色':
            ...
            if len(result) >= 1:
                orgimg = cv2ImgAddText(orgimg, result, rect_area[0] - height_area, rect_area[1] - height_area - 10, (0, 255, 0),
                                    height_area)
    # print(result_str)
    return result_str,orgimg


def detect_and_rec():
    begin = time.time()
    parser = argparse.ArgumentParser()
    parser.add_argument('--detect_model', type=str, default='onnx/plate_detect.onnx', help='model.pt path(s)')  # 检测模型
    parser.add_argument('--rec_model', type=str, default='onnx/plate_rec_color.onnx', help='model.pt path(s)')  # 识别模型
    ...
    for pic_ in file_list:
        ...
        result_str,ori_img = draw_result(img0, result_list)
        img_name = os.path.basename(pic_)
        save_img_path = os.path.join(save_path, img_name)
        with open(save_img_path, 'wb') as f:
            f.write(cv2.imencode('.jpg', ori_img)[1].tobytes())
